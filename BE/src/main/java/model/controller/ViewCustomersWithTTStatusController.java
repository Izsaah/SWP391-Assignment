...
public class ViewCustomersWithTTStatusController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<Map<String, Object>> customers = paymentService.getAllCustomersWithTTStatus(dealerStaffId);

            if (customers == null || customers.isEmpty()) {
                ResponseUtils.success(response, "No customers with TT status found", Collections.emptyList());
            } else {
                ResponseUtils.success(response, "Full payment customers retrieved successfully", customers);
            }
            ...
            e.printStackTrace();
            ResponseUtils.error(response, "Failed to retrieve customers with TT status: " + e.getMessage());
        }
    }
}